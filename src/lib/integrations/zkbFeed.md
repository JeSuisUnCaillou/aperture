...
- **Freshness guard:** a decoded kill older than `ZKB_FEED_MAX_KILL_AGE_MS` (by ESI `killmail_time`), or with a missing/unparseable time, is dropped before correlation and logged `zkb.stale_skipped` (`killmailId`, `ageMs`; `ageMs` is null for an undatable kill). Guards against zKB placing reprocessed / late-submitted killmails into the live sequence, which a healthy forward walk would otherwise flash.
...
### Depends On
- `@/db/client` (`db`), `@/db/schema` (`apMap`, `apMapSystem`), `./zkb` (`zkbKillSchema`, `ZkbKill`), `@/lib/realtime/protocol` (`SystemNotificationLoad`), `@/lib/log/logger` (`getLogger`), `aperture.config` (feed cadences, base URL, UA, channel prefix), `drizzle-orm`, `zod`.
