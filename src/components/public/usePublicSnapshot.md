...
### usePublicSnapshot(token: string, initial: PublicMapViewData, initialBuild: string): { data: PublicMapViewData; status: PublicFeedStatus; updatedAt: number; build: string }
...
- `initialBuild` — the build id of the process that rendered the page, seeding `build` so the very first refetch can already reveal a redeploy.
...
- `build` — the build id reported by the last successful refetch.
