...
### Response
`{ ok: true, data: WormholeCatalogEntry[] }` — every `universe_wormhole` row (`typeId`, `name`, `sourceClasses`, `targetClass`, `targetSystemId`, `targetSystemName`, `jumpMassClass`) ordered by code, via `wormholeCatalog()`. `targetSystemId` / `targetSystemName` carry a fixed destination (J377 → Turnur); null for normal holes. `401 { ok: false, error }` when signed out.
