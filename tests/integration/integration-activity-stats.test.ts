...
import { apCharacter, apIntegrationToken, apMap, apMapEvent, apUser } from '@/db/schema';
...
describe.skipIf(!run)('Integrations — activity stats (real Postgres)', () => {
  ...
  it('reports coverage as the min/max rollup day within the corp scope', async () => {
    const stats = await loadIntegrationActivityStats({
      corporationId: CORP_A,
      characterIds,
      granularity: 'weekly',
    });
    expect(stats.coverage).toEqual({ earliest: '2026-06-30', latest: '2026-07-06' });
  });
